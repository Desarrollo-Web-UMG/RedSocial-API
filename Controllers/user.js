...
const { revokedTokens } = require("../middlewares/auth"); // Importa la lista negra desde el middleware
...
const login = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ email: params.email })
            .select({ name: 1, surname: 1, nick: 1, email: 1, role: 1, image: 1, password: 1, descripcion: 1 })
            .exec();
...
const logout = (req, res) => {
    try {
        ...
        // Decodificar el token usando jwt-simple y la clave secreta de jwt.js
        const decodedToken = jwtSimple.decode(token, jwt.claveSecreta);
        ...
        // Responder que el logout fue exitoso
        return res.status(200).send({
            status: "success",
        ...
        return res.status(500).send({
            status: "error",
            message: "Error al intentar revocar el token",
            error: error.message
        });
    }
};
...
const upload = async (req, res) => {
    try {
        ...
        // Comprobar extensión
        if (extension != "png" && extension != "jpg" && extension != "JPG" && extension != "jpeg" && extension != "gif" && extension != "webp") {
